...
func (n *node) equal(y *node) (string, bool) {
	...
	// fmt.Println("nhv:", nhv, n.path)
	// fmt.Println("yhv:", yhv, y.path)
	if nhv != yhv {
...
func Test_findRoute_Middleware(t *testing.T) {
	...
	mwsRoute := []struct {
	...
	}{
		{
			method: http.MethodGet,
			path:   "/a/b",
			mws:    []Middleware{mdlBuilder('a'), mdlBuilder('b')},
		},
		{
			method: http.MethodGet,
			path:   "/a/*",
			mws:    []Middleware{mdlBuilder('a'), mdlBuilder('*')},
		},
		{
			method: http.MethodGet,
			path:   "/a/b/*",
			mws:    []Middleware{mdlBuilder('a'), mdlBuilder('b'), mdlBuilder('*')},
		},
		{
			method: http.MethodPost,
			path:   "/a/b/*",
		...
		},
		{
			method: http.MethodPost,
			path:   "/a/*/c",
			mws:    []Middleware{mdlBuilder('a'), mdlBuilder('*'), mdlBuilder('c')},
		},
		{
			method: http.MethodPost,
			path:   "/a/b/c",
			mws:    []Middleware{mdlBuilder('a'), mdlBuilder('b'), mdlBuilder('c')},
		},
		{
			method: http.MethodDelete,
			path:   "/*",
			mws:    []Middleware{mdlBuilder('*')},
		},
		{
			method: http.MethodDelete,
			path:   "/",
	...
	r := newRouter()
	for _, mdlRoute := range mwsRoute {
		r.addRoute(mdlRoute.method, mdlRoute.path, nil, mdlRoute.mws...)
	}
	//fmt.Printf("%#v\n", r.trees["GET"].children["a"].starChild.mws)
	testCases := []struct {
		name   string
		method string
		path   string
